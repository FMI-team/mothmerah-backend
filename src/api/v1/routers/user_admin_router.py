...
# (هذا الراوتر يعمل كنقطة تجميع لجميع الراوترات الإدارية المتخصصة)

from fastapi import APIRouter # استيراد المكونات الأساسية لـ FastAPI
# لا داعي لاستيراد Depends, status, HTTPException هنا
# ولا داعي لاستيراد Session, datetime, date, base, UserModel, UUID هنا
# لا داعي لاستيراد Schemas أو Services هنا مباشرة

# استيراد الراوترات الإدارية المتخصصة التي سيتم دمجها
from src.api.v1.routers import (
    admin_core_users_router,       # لإدارة المستخدمين الأساسية (Admin /users)
    ...
    admin_auctions_router,         # لإدارة المزادات كمسؤول
    unit_of_measure_admin_router   # لإدارة وحدات القياس كمسؤول
)
...
#       مثال: from src.api.v1.routers import admin_pricing_management_router


# تعريف الراوتر الرئيسي لإدارة النظام من جانب المسؤولين.
# هذا الراوتر سيكون بمثابة "المدخل" لجميع أقسام الإدارة في لوحة التحكم.
router = APIRouter() # لا نضع له prefix هنا، بل في main.py عند تضمينه كـ /api/v1/admin
...
#       مثال: router.include_router(admin_pricing_management_router.router)
